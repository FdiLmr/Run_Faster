...
def run_cli() -> None:
    ...
    try:
        ...
        try:
            ...
            print(
                "Please ensure model files are present in the marathon_predictor directory."
            )
            return
        ...
        try:
            ...
            if "model1" in predictions:
                print(
                    f"  Single Race Model: {format_runtime(predictions['model1'] * 60)}"
                )
            if "model2" in predictions:
                print(
                    f"  Dual Race Model:   {format_runtime(predictions['model2'] * 60)}"
                )
