...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    f'mysql+pymysql://{os.environ.get("DB_USER")}:{os.environ.get("DB_PASS")}@{os.environ.get("DB_HOST")}/{os.environ.get("DB_NAME")}'
)
        ...
        if "metadata_pbs" not in existing_tables:
            ...
            with db.engine.connect() as conn:
                ...
                if not has_constraint:
                    logger.info(
                        "Recreating metadata_pbs table with uniqueness constraint"
                    )
        ...
        activities_data = [
            {
                ...
                "elapsed_time_raw": a.elapsed_time
                // 60,  # Raw numeric value for sorting
                ...
                "avg_hr": (
                    f"{a.average_heartrate:.0f}" if a.average_heartrate else "N/A"
                ),
...
def activity_detail(activity_id):
    ...
    return render_template(
        "activity_detail.html", activity=record, athlete_id=athlete_id
    )
