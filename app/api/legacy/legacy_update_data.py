...
ACTIVITIES_LIMIT = 90 if DEBUG_MODE else 90
...
def save_activity_data(
    athlete_id: int, activities: list, timestamp: str = None
) -> None:
            ...
            if athlete_id != 0 and row["status"] == "none":
                ...
                try:
                    ...
                    activities_to_process = (
                        ACTIVITIES_LIMIT  # Changed from hardcoded 90
                    )
                    all_activities = []
                    page = 1
                    ...
                    while True:
                        ...
                        unprocessed = get_unprocessed_activities(
                            all_activities, existing_ids, activities_to_process
                        )
                    ...
                    has_more_activities = (
                        len(all_activities) > len(existing_ids) + activities_to_process
                    )
                    ...
                    if len(unprocessed) > 0:
                        ...
                        for activity in unprocessed[:activities_to_process]:
                            ...
                            if os.path.exists(file_path):
                                ...
                                logger.info(
                                    f"Fetching activity {activity_id} from cached file"
                                )
                            else:
                                ...
                                current_api_calls += (
                                    1  # Only count an API call if we had to fetch it
                                )
                                logger.info(
                                    f"Fetching activity {activity_id} from Strava"
                                )
                            ...
                            try:
                                ...
                                streams_file_path = os.path.join(
                                    athlete_dir, f"{activity_id}_streams.json"
                                )

                                if os.path.exists(streams_file_path):
                                    # If the file already exists, load the JSON data from disk
                                    with open(
                                        streams_file_path, "r", encoding="utf-8"
                                    ) as f:
                                        streams_data = json.load(f)
                                    logger.info(
                                        f"Fetching activity streams for {activity_id} from cached file"
                                    )
                                else:
                                    logger.info(
                                        f"Fetching activity streams for {activity_id} from Strava"
                                    )
                                    # Fetch the streams using the existing function
                                    streams_data = fetch_activity_streams(
                                        activity_id, headers
                                    )

                                    # Save the streams data to file
                                    with open(
                                        streams_file_path, "w", encoding="utf-8"
                                    ) as f:
                                    ...
                                    current_api_calls += 1
                                    logger.info(
                                        f"Successfully fetched and saved streams for activity {activity_id}"
                                    )
                            ...
                            be_columns = {
                                col: None for col in best_effort_mapping.values()
                            }
                            ...
                            try:
                                activity = Activity(
                                    ...
                                    start_date_local=(
                                        datetime.strptime(
                                            this_response.get("start_date_local"),
                                            "%Y-%m-%dT%H:%M:%SZ",
                                        )
                                        if this_response.get("start_date_local")
                                        else None
                                    ),
                                    ...
                                    location_country=this_response.get(
                                        "location_country"
                                    ),
                                    achievement_count=this_response.get(
                                        "achievement_count"
                                    ),
                                    ...
                                    average_cadence=this_response.get(
                                        "average_cadence"
                                    ),
                                    ...
                                    weighted_average_watts=this_response.get(
                                    ...
                                    heartrate_opt_out=this_response.get(
                                        "heartrate_opt_out"
                                    ),
                                    display_hide_heartrate_option=this_response.get(
                                    ...
                                    similar_activities=this_response.get(
                                        "similar_activities"
                                    ),
                                    available_zones=this_response.get(
                                        "available_zones"
                                    ),
                                    splits_metric=this_response.get("splits_metric"),
                                    splits_standard=this_response.get(
                                        "splits_standard"
                                    ),
                                    laps=this_response.get("laps"),
                                    photos=this_response.get("photos"),
                                    stats_visibility=this_response.get(
                                        "stats_visibility"
                                    ),
                                ...
                                logger.error(
                                    f"Error storing activity {activity_id}: {e}"
                                )
                                continue
                        ...
                        try:
                            db.session.commit()
                            logger.info(
                                f"Successfully stored {new_activities_count} new activities"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error committing activities to database: {e}"
                            )
                            db.session.rollback()
                            raise
                    else:
                        logger.info(
                            f"No new activities to process for athlete {athlete_id}"
                        )
                ...
                if has_more_activities:
                    print(
                        f"There are more activities to process for athlete {athlete_id}"
                    )
