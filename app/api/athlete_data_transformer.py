...
def load_latest_athlete_data(athlete_id: int) -> dict:
    ...
    try:
        ...
        if not existing_activities_df.empty:
            ...
            all_activities = [
                act for act in all_activities if act.get("id") in valid_ids
            ]
            logger.info(
                f"After filtering, {len(all_activities)} activities remain (present in the DB)."
            )
        else:
            logger.info(
...
def get_athlete_zones(athlete_data: dict) -> List[int]:
    ...
    try:
        ...
        return [round(190 * 0.6), round(190 * 0.7), round(190 * 0.8), round(190 * 0.9)]
...
def process_pb_blocks(
    activities: List[dict], athlete_id: str, zones: List[int], hr_regressor
...
def process_time_based_blocks(
    activities: List[dict],
    athlete_id: str,
    zones: List[int],
    hr_regressor,
    block_duration_days: int = 91,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Process activities into time-based training blocks when PB-based blocks are insufficient."""
    metadata_blocks = pd.DataFrame()
    features_activities = pd.DataFrame()
    features_weeks = pd.DataFrame()
    ...
    activities_sorted = sorted(activities, key=lambda x: x.get("start_date", ""))

    # Create time-based blocks
    start_date = datetime.strptime(activities_sorted[0]["start_date"][:10], "%Y-%m-%d")
    end_date = datetime.strptime(activities_sorted[-1]["start_date"][:10], "%Y-%m-%d")

    current_date = start_date
    block_num = 0

    while current_date < end_date:
        block_end = current_date + timedelta(days=block_duration_days)
        block_id = f"time_block_{block_num}"

        # Get activities in this time period
        block_activities = []
        for activity in activities_sorted:
            activity_date = datetime.strptime(activity["start_date"][:10], "%Y-%m-%d")
            if current_date <= activity_date < block_end:
                block_activities.append(activity)

        # Only process blocks with sufficient activities
        if len(block_activities) >= MIN_ACTIVITIES_PER_BLOCK:
            # Add block metadata (without PB-specific data)
            block_metadata = {
                "athlete_id": athlete_id,
                "vdot": 0.0,  # Default value
                "vdot_delta": 0.0,  # Default value
                "predicted_marathon_time": 0.0,  # Default value
                "pb_date": block_end,  # Use block end as reference date
                "block_id": block_id,
            }
            metadata_blocks = pd.concat(
                [metadata_blocks, pd.DataFrame([block_metadata])], ignore_index=True
            ...
            # Process activities by week
            block_features, block_weeks = process_activity_block(
                block_activities,
                {"id": athlete_id},
                athlete_id,
                zones,
            ...
            features_activities = pd.concat(
                [features_activities, block_features], ignore_index=True
            )
            features_weeks = pd.concat([features_weeks, block_weeks], ignore_index=True)
...
def calculate_activity_proportions(
    activities_df: pd.DataFrame, activity_type: Union[int, List[int]]
) -> float:
...
def calculate_relative_proportion(
    block_prop: float, total_prop: float
) -> Optional[float]:
...
def get_activity_type_metrics(
    block_activities: pd.DataFrame, all_activities: pd.DataFrame
) -> Dict[str, float]:
...
def calculate_training_metrics(
    block_weeks: pd.DataFrame, athlete_weeks: pd.DataFrame
) -> Dict:
...
def process_metadata_pbs(activities, athlete_id):
    ...
    for distance_category, efforts in all_pbs.items():
        ...
        try:
            pb_date = datetime.strptime(best_effort.get("start_date")[:10], "%Y-%m-%d")
        except Exception:
            pb_date = None
        ...
        existing_pb = (
            ...
            .filter_by(athlete_id=str(athlete_id), distance_category=distance_category)
            .first()
        )

        if existing_pb:
            # Only update if the new PB is faster
            if best_effort.get("elapsed_time", float("inf")) < existing_pb.elapsed_time:
...
def transform_athlete_data(
    ...
    try:
        ...
        if not_nan_rows is not None:
            average_paces_and_hrs = pd.concat(
                [average_paces_and_hrs, not_nan_rows], ignore_index=True
            )
        ...
        # If PB-based processing yielded insufficient data, use time-based blocks as fallback
        if len(features_activities) < 20:  # Threshold for insufficient data
            logger.info(
                f"PB-based processing yielded only {len(features_activities)} activities. Using time-based blocks as fallback."
            )
            time_metadata_blocks, time_features_activities, time_features_weeks = (
                process_time_based_blocks(activities, athlete_id, zones, regressor)
            )

            # Combine or replace with time-based data
            if len(time_features_activities) > len(features_activities):
                metadata_blocks = time_metadata_blocks
                features_activities = time_features_activities
                features_weeks = time_features_weeks
                logger.info(
                    f"Replaced with time-based processing: {len(features_activities)} activities"
                )
        ...
        # Update race predictions based on latest PBs
