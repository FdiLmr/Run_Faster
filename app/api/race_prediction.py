...
def calculate_athlete_predictions(athlete_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not pbs_df.empty:
            ...
            pb_5k_all = athlete_pbs[
                athlete_pbs["distance_category"] == "5K"
            ].sort_values("elapsed_time")
            # Get all 10K performances and sort by elapsed_time to find the fastest
            pb_10k_all = athlete_pbs[
                athlete_pbs["distance_category"] == "10K"
            ].sort_values("elapsed_time")
...
def get_latest_prediction(athlete_id: str) -> Optional[Dict]:
    ...
    try:
        prediction = (
            RacePrediction.query.filter_by(athlete_id=athlete_id)
            .order_by(desc(RacePrediction.created_at))
            .first()
        )
