...
This module provides functionality for extracting and processing data
from individual activities, including both running and non-running activities.
"""
...
def get_run_activity_data(activity: dict, zones: List[int], hr_regressor) -> Tuple:
    ...
    additional_data = {
        ...
        "elevation": safe_get(activity, "elev_high", 0)
        - safe_get(activity, "elev_low", 0),
    ...
    try:
        ...
        if laps:
            ...
            for signal_type in ["hr", "elevation", "pace"]:
                if lap_data[signal_type]:
                    signal_metrics[signal_type] = calculate_signal_metrics(
                        lap_data[signal_type]
                    )
            ...
            if lap_data["hr"]:
                ...
                if basic_data[4] is None and hr_regressor is not None:
                    try:
                        basic_data[4] = hr_regressor.predict(
        ...
        logger.error(
            f"Error processing run data for activity {activity.get('id', 'unknown')}: {e}"
        )
