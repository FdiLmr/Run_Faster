...
def prepare_features(
    features_blocks: pd.DataFrame, athlete_id: Optional[str] = None
    ...
    try:
        ...
        if athlete_id:
            athlete_data = features_blocks[
                features_blocks["athlete_id"] == int(athlete_id)
            ]
        ...
        y_absolute = pd.to_numeric(features_blocks.iloc[:, -2], errors="coerce").fillna(
            0
        )
...
def train_random_forest(
    X: np.ndarray, y: np.ndarray, n_estimators: int = 50
    ...
    try:
        ...
        for index in indices:
            ...
            model_outputs = pd.concat(
                [model_outputs, pd.DataFrame([row_data])], ignore_index=True
            )
...
def generate_shap_plots(
    ...
    try:
        ...
        shap.summary_plot(
            shap_values, pd.DataFrame(X, columns=feature_names), show=False
        )
        plt.tight_layout()
        summary_filename = (
            f"static/shap_plots/{plot_type}_summary_{athlete_id}.png"
            if athlete_id
            else f"shap_plots/{plot_type}_summary.png"
        )
        ...
        plt.tight_layout()
        bar_filename = (
            f"static/shap_plots/{plot_type}_bar_{athlete_id}.png"
            if athlete_id
            else f"shap_plots/{plot_type}_bar.png"
        )
...
def train_model(athlete_id: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        model_outputs = pd.concat(
            [
                ...
                calculate_feature_importance(
                    regressor_abs, feature_names, "absolute_vdot", score_abs, athlete_id
                ),
            ],
            ignore_index=True,
        )
        ...
        try:
            generate_shap_plots(
                regressor_abs, X, feature_names, "absolute_vdot", athlete_id
            )
            generate_shap_plots(
                regressor_change, X, feature_names, "vdot_change", athlete_id
            )
        except Exception as e:
