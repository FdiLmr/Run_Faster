...
class Activity(db.Model):
    ...
    manual = db.Column('is_manual', db.Boolean, nullable=True)
    private = db.Column('is_private', db.Boolean, nullable=True)
...
class MetadataPB(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "athlete_id", "distance_category", name="uix_athlete_id_distance_category"
        ),
    )

    def __repr__(self):
        return (
            f"<MetadataPB {self.athlete_id} {self.distance_category} PB: "
...
class RacePrediction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    athlete_id = db.Column(db.String(100), nullable=False)
    riegel_exponent = db.Column(
        db.Float, nullable=False
    )  # Calculated personalized exponent
